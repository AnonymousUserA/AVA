...
def baidu(image_path):
    APP_ID = '31757321'
    API_KEY = '1oloczOQKwuYGE8nKkTN0Kkf'
    SECRET_KEY = 'GLZGjjygBGkTVaosjTBYkUwum7tgifGG'
...
def initpopluation(alpha, latent, nothing, image_name, output_path, configs_all_semantic, noise_in, first_dis, loss_dis_init, original_image, api, n = 300):
    ...
    #     if np.abs(i) < 2:
    #         weights.append(0.4)
    #     elif np.abs(i) >= 2 and np.abs(i) < 3:
    #         weights.append(0.3)
    #     elif np.abs(i) >= 3 and np.abs(i) < 4:
    #         weights.append(0.2)
    #     elif np.abs(i) >= 4:
    #         weights.append(0.1)
    # for i in np.arange(-10.0, 10.0, 0.01):
    #     if np.abs(i) < 4:
    #         weights.append(0.4)
    #     elif np.abs(i) >= 4 and np.abs(i) < 6:
    #         weights.append(0.3)
    #     elif np.abs(i) >= 6 and np.abs(i) < 8:
    #         weights.append(0.2)
    #     elif np.abs(i) >= 8:
    #         weights.append(0.1)
    # for i in np.arange(-3.0, 3.0, 0.01):
    #     if np.abs(i) < 1.5:
    #         weights.append(0.4)
    #     elif np.abs(i) >= 1.5 and np.abs(i) < 2:
    #         weights.append(0.3)
    #     elif np.abs(i) >= 2 and np.abs(i) < 2.5:
    #         weights.append(0.2)
    #     elif np.abs(i) >= 2.5:
    #         weights.append(0.1)

    for i in np.arange(-8.0, 8.0, 0.01):
        if np.abs(i) < 3:
            weights.append(0.4)
        elif np.abs(i) >= 3 and np.abs(i) < 5:
            weights.append(0.3)
        elif np.abs(i) >= 5 and np.abs(i) < 7:
            weights.append(0.2)
        elif np.abs(i) >= 7:
            weights.append(0.1)
    ...
    for i in range(n):
        sample.append(random.choices(list(np.arange(-8.0, 8.0, 0.01)), k=channel_number, weights=weights))
    ...
    for style_code in all_style_code:
        ...
        # for model
        if api in ["GramNet", "patch_forensics"]:
        ...
        loss.append(loss_id.item())
        times += 1
    # get n min loss style code
    popluation = []
    loss = np.array(loss)
    loss_index = np.argsort(loss)[:200]
    # weights_v2 = []
    # for i in loss_index:
    #     weights_v2.append(loss[i])
    # popluation_index = random.choices(list(loss_index), k=200, weights=weights_v2)
    ...
    for i in loss_index:
        ...
        # for academic
        img_gen, _ = g_ema([all_style_code[i]], input_is_latent=True, randomize_noise=False, noise=noise_in,
                           input_is_stylespace=True)
        img_tensor = transforms.Resize((299, 299))(((img_gen + 1) / 2).clamp_(0, 1))
        ...
        elif api is "CNNDetection":
            _, pred = CNNDetection(img_tensor, model, loss_func)
            pred = pred.item()
            fitness_score.append(pred)
        elif api is "F3-Net":
            _, pred = F3_Net(img_tensor, model, loss_func)
            pred = pred.item()
            fitness_score.append(pred)

    return popluation_stylecode, loss_index, fitness_score
...
def get_parents(k, fitness_score):
    ...
    for score in fitness_score:
        weights.append(1.1-score)
    # weights = 1- fitness_score
...
# Attack
def main(nothing, image_name, output_path, output_inversion_path, configs_all_semantic, latent_code_init, noise, original_image, lambda_dis, lambda_id, attribute_channel, attribute_edit, lr_list, first_dis, loss_dis_init, result_path, mutate_prob, api):
    if nothing:
        # define working file
...
if __name__ == '__main__':
    """
        ...
        **************************************
    """
    print("range:8, mutate:0.1, number:200")
    dir_path = "/8T/xiangtao/old/dataset/test_ff++" #/8T/xiangtao/new/dataset/original_1024_100_299
    # dir_path = "/8T/xiangtao/new/dataset/inversion/299/stylegan/images/with_noise_100"
    latent_path = "/8T/xiangtao/old/dataset/inversion/ff++/latents/with_noise_100"  #/8T/xiangtao/new/dataset/inversion/stylegan/latents/with_noise_100
    noise_path = "/8T/xiangtao/old/dataset/inversion/ff++/noise/with_noise_100"  #/8T/xiangtao/new/dataset/inversion/stylegan/noise/with_noise_100
    output_path = "/8T/xiangtao/new/dataset/results/ff++/baidu_tmp/"
    output_inversion_path = "/8T/xiangtao/new/dataset/results/ff++/"
    result_path = "/8T/xiangtao/new/dataset/results/ff++/baidu_8_200_0.1/"
            ...
            for image in os.listdir(latent_path):
                ...
                for mutate_prob in mutate_probs:
                    ...
                    success = main(nothing, image_name, tmp_output_path, output_inversion_path, configs_all_semantic,
                                      latent_code, noise, original_image, lambda_dis, lambda_id, attribute_channel, attribute_edit, lr_list, first_dis, loss_dis_init, result_path, mutate_prob, api)
                    if success:
                        break
